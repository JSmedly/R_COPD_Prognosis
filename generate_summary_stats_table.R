...
data_sds.sex
data_means.sex
data_pvalues <- sapply(data %>% select(all_of(preds_cts)), function(i) t.test(i ~ sex, data=data, alternative="two.sided")$p.value)
data_pvalues <- t(data.frame(data_pvalues))
rownames(data_pvalues) <- "p.value"

data_means <- rbind(data_means.all, data_means.sex)
data_sds <- rbind(data_sds.all, data_sds.sex)
data_means <- rbind(data_means, data_sds, data_pvalues)
...
#####################################################
# Create counts and percents table
...
data_counts.sex <- newdata %>%
  ...
  summarise(across(everything(), ~sum(., na.rm=TRUE))) %>%
  select(-sex.0) %>%
  mutate(n=sex_counts, .after=sex.1) %>%
  mutate(sex.1 = recode_factor(sex.1, `0`="Female", `1`="Male")) %>% #sex.0=1 is one-hot encoding for "female"
  column_to_rownames("sex.1")
data_counts.sex

# Apply chisquared tests
data_counts_pvalues <- sapply(categ_data %>% select(-sex), function (i) chisq.test(ftable(sex ~ i, data=categ_data))$p.value)
data_counts_pvalues <- data.frame(t(data_counts_pvalues))
rownames(data_counts_pvalues) <- "p.value"
colnames(data_counts_pvalues) <- c(
...
bind_rows(data_counts.all, data_counts.sex, data_counts_pvalues)
...
# Combine counts, percents, 
data_counts <- bind_rows(data_counts, data_pcts, data_counts_pvalues)

#####################################################
# Combine table into summary statistics
...
summary_statistics <- summary_statistics %>% mutate(p.value = ifelse(p.value<0.05, "*", ""))
# Format
